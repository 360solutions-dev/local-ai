...
import { useFlatConversations, useAllIndexedFiles, useOllamaModels, useSystemHealth, useModelConfig } from "@/hooks/use-chat";
...
export default function DashboardClient() {
  ...
  const { data: files, isLoading: loadingFiles } = useAllIndexedFiles();
  ...
  // Show the model the user actually selected (ModelConfig.chat_model). Only
  // fall back to the first installed model if nothing is configured — never
  // show an arbitrary list-order model that flips between refreshes.
  const configuredModel = (modelConfig?.chat_model || "").trim();

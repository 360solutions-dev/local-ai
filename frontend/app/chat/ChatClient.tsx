...
import { Archive, ArrowRight, Check, Clock, Copy, Ellipsis, Loader2, Mic, Pencil, Pin, PinOff, Trash2, Upload } from "lucide-react";
import Sidebar from "@/components/layout/Sidebar";
import VoiceInput from "@/components/ui/VoiceInput";
import Toast from "@/components/ui/Toast";
import CloudModelWarning from "@/components/ui/CloudModelWarning";
import { isCloudModel } from "@/lib/ollama-classifier";
...
// Shows how long a response took. While `end` is null the stream is still
// running, so it ticks live; once `end` is set it freezes on the final value.
function ResponseTimer({ start, end, label }: { start: number; end: number | null; label: string }) {
  ...
  const elapsed = Math.max(0, (end ?? now) - start) / 1000;
  return (
    <span
      className="inline-flex items-center gap-1 font-mono text-[0.68rem] text-text-dim font-normal"
      title={label}
    >
      <Clock size={11} />
      {elapsed.toFixed(1)}s
...
function groupByDate<T extends { id: number; title: string; created_at: string }>(
  items: T[],
...
export default function ChatClient() {
  ...
  // Per-conversation abort controllers + active turns. Keyed by conversation
  // id so multiple chats can stream concurrently — switching chats does NOT
  // cancel an in-flight generation; only an explicit Stop on that chat does.
  const abortControllersRef = useRef<Map<number, AbortController>>(new Map());
  const activeTurnsRef = useRef<Map<number, { turnId: string }>>(new Map());
  ...
  const { streamSend, streamingIds, timings } = useStreamingSend();
  ...
  const { data: archivedConvs = [] } = useArchivedConversations(archivedModalOpen);
  ...
  // Track which conversation an upload is currently associated with.
  // useUploadFile.isPending is a global mutation flag — without this, the
  // "uploading" spinner would appear in every chat's panel while a single
  // upload is in flight elsewhere. Set when we start an upload, clear in
  ...
  const isUploadingHere = uploadFile.isPending && uploadingChatId === activeChatId;
  ...
  const { data: modelConfig, isFetched: modelConfigFetched } = useModelConfig();
  ...
  const [cloudWarningName, setCloudWarningName] = useState<string | null>(null);
   ...
   */
  // Ensure there's an active conversation, creating ONE if needed. A shared
  // promise guards against concurrent callers each making their own chat — the
  // bug where selecting multiple files for a new chat split them across several
  // chats (one per file) instead of all landing in one.
  const creatingChatRef = useRef<Promise<number> | null>(null);
  const ensureActiveChat = useCallback(async (): Promise<number | null> => {
    if (activeChatId) return activeChatId;
    if (!creatingChatRef.current) {
      creatingChatRef.current = createConversation.mutateAsync(undefined).then((conv) => {
        setActiveChatId(conv.id);
        return conv.id;
      });
    }
    const pending = creatingChatRef.current;
    try {
      ...
      // Allow a future brand-new chat to create again once this one settled.
      if (creatingChatRef.current === pending) creatingChatRef.current = null;
    }
  }, [activeChatId, createConversation, showToast, t]);

  const ensureChatThenUpload = useCallback(
    async (file: File, cidOverride?: number) => {
      const cid = cidOverride ?? (await ensureActiveChat());
      if (!cid) return;
      setUploadProgress({ fileName: file.name, percent: 0, phase: "uploading" });
      setUploadingChatId(cid);
      uploadFile.mutate(
        {
          file,
          conversationId: cid,
          onProgress: (update) => {
            setUploadProgress((prev) =>
              prev ? { ...prev, percent: update.percent, phase: update.phase } : prev,
            );
          },
        },
        {
    ...
    [ensureActiveChat, uploadFile, showToast, t],
  );
  ...
  const processFileForUpload = useCallback(
    async (file: File, cidOverride?: number) => {
      ...
      if (!res.ok || !res.data) {
        ensureChatThenUpload(file, cidOverride);
        return;
      }
  ...
  // Slowly advance the bar while the server indexes after upload bytes are sent
  useEffect(() => {
    if (!isUploadingHere || uploadProgress?.phase !== "indexing") return;
    const id = window.setInterval(() => {
      setUploadProgress((prev) => {
        if (!prev || prev.phase !== "indexing" || prev.percent >= 98) return prev;
        return { ...prev, percent: prev.percent + 1 };
      });
    }, 400);
    return () => window.clearInterval(id);
  }, [isUploadingHere, uploadProgress?.phase]);
  ...
  // Keep the chat header in sync with the user's saved model preference.
  // Settings → "Use this model" updates modelConfig.chat_model; this effect
  // propagates that change here so the active model badge and outgoing
  // requests pick it up without a page reload. The in-session model picker
  // (Choose model overlay) still wins until the saved preference changes again.
  const lastSyncedConfigRef = useRef<string>("");
  useEffect(() => {
    const configured = (modelConfig?.chat_model || "").trim();
    if (configured && configured !== lastSyncedConfigRef.current) {
      // Wait until the installed-models list has loaded before resolving the
      // saved model. Marking it "synced" before the list arrived was the bug:
      // the match failed, the flag was set, and on the next render the fallback
      // picked the first model — so a refresh showed the wrong model.
      if (ollamaModels.length === 0) return;
      const match = ollamaModels.find((m) => m.name === configured || m.id === configured);
      lastSyncedConfigRef.current = configured;
      if (match) {
        setSelectedModel(match.id);
        return;
      }
      // Saved model is no longer installed — fall through to the default below.
    }
    // Fall back to the first installed model only once the saved config has
    // loaded, so we never flash the wrong model before the saved one arrives.
    if (!selectedModel && ollamaModels.length > 0 && modelConfigFetched) {
      setSelectedModel(ollamaModels[0].id);
    }
  }, [modelConfig?.chat_model, ollamaModels, selectedModel, modelConfigFetched]);

  // Select a chat model from the in-chat "Choose a model" overlay AND persist
  // it to ModelConfig.chat_model. Without persisting, the choice lived only in
  // local state and was lost on refresh (dashboard/header would fall back to
  // the first installed model). Persisting makes the active model consistent
  // across the chat header, dashboard, and Model Engines page, and survives reload.
  function selectChatModel(model: { id: string; name: string }) {
    setSelectedModel(model.id);
    setModelOverlayOpen(false);
    const ollamaProvider =
      providers.find((p) => p.type === "ollama" && p.is_connected) ||
      providers.find((p) => p.type === "ollama");
    updateModelConfig.mutate({
      chat_model: model.name,
      chat_provider_id: ollamaProvider?.id ?? null,
    });
  }
  ...
  async function handlePullModel() {
    ...
    // Models download from the public Ollama registry — fail fast offline.
    if (!(await checkOnline())) {
      setPullError(t("chat.noInternet"));
      return;
    }
  ...
  function handleConfirmCloudPull() {
    const name = cloudWarningName;
    setCloudWarningName(null);
  ...
  // Filtered file list for autocomplete. A synthetic "@all" entry (when there
  // is more than one file) lets the user explicitly query across ALL files.
  const ALL_MENTION_ID = "__all__";
  const fileMatches = indexedFiles.filter((f) =>
    f.name.toLowerCase().includes(fileMentionQuery),
  );
  const showAllOption =
    indexedFiles.length > 1 &&
    (fileMentionQuery === "" || "all".includes(fileMentionQuery));
  const mentionFiles: Array<{ id: string; name: string; type?: string; chunks?: number }> =
    showAllOption ? [{ id: ALL_MENTION_ID, name: "all" }, ...fileMatches] : fileMatches;
  ...
  async function handleSend(textOverride?: string) {
    ...
    // "@all" (or no mention) → send no filter so the backend searches across
    // ALL files in this chat. A specific @file scopes to that one file.
    const isAllMention = taggedFile?.id === ALL_MENTION_ID;
    const fileFilter = taggedFile && !isAllMention ? taggedFile.name : undefined;
    ...
    activeTurnsRef.current.set(chatId, { turnId });
    ...
    try {
      ...
      // Clean up this chat's controller/turn only — other chats may still be
      // streaming concurrently.
      abortControllersRef.current.delete(chatId);
      activeTurnsRef.current.delete(chatId);
    }
  }
  ...
  function handleStop() {
    // Stop only the currently-viewed chat's stream — other chats keep running.
    ...
    if (activeTurnsRef.current.has(activeChatId)) {
      stripOptimisticAssistantFromCache(activeChatId);
      activeTurnsRef.current.delete(activeChatId);
    }
  }
  ...
  function onKeyDown(e: KeyboardEvent<HTMLTextAreaElement>) {
    ...
    if (e.key === "Enter" && !e.shiftKey) {
      e.preventDefault();
      // Block send while a file is still uploading/indexing in this chat —
      // otherwise the prompt runs before the document is searchable.
      if (isUploadingHere) return;
      handleSend();
    }
  }
  ...
  function handleNewChat() {
    ...
    // Do NOT abort an in-flight stream from the previous chat. The user only
    // asked to start a new chat — the previous generation keeps running in the
    // background (it writes to its own conversation's cache), the sidebar shows
    // a loader on it, and the full answer is there when they return.
  ...
  function handleTogglePin(convId: number, currentlyPinned: boolean) {
    setMenuOpenId(null);
    setPinned.mutate(
      { conversationId: convId, pinned: !currentlyPinned },
      {
        onSuccess: () => showToast(currentlyPinned ? t("chat.unpinned") : t("chat.pinned")),
        onError: (err) => showToast(err instanceof Error ? err.message : t("chat.actionFailed")),
      },
    );
  }

  function handleArchiveConversation(convId: number) {
    setMenuOpenId(null);
    // Archived chats leave the default list — if the open one is archived,
    // drop back to the empty new-chat state.
    setArchived.mutate(
      { conversationId: convId, archived: true },
      {
        onSuccess: () => {
          if (activeChatId === convId) setActiveChatId(null);
          showToast(t("chat.archived"));
        },
        onError: (err) => showToast(err instanceof Error ? err.message : t("chat.actionFailed")),
      },
    );
  ...
  function handleUnarchiveConversation(convId: number) {
    setArchived.mutate(
      { conversationId: convId, archived: false },
      {
        onSuccess: () => showToast(t("chat.unarchived")),
        onError: (err) => showToast(err instanceof Error ? err.message : t("chat.actionFailed")),
      },
    );
  }
  ...
  // Pinned chats float to the top in their own "Pinned" group; the rest are
  // grouped by date as before. (Archived chats are already excluded server-side.)
  const pinnedConvs = conversations.filter((c) => c.pinned);
  const unpinnedConvs = conversations.filter((c) => !c.pinned);
  const grouped = [
    ...(pinnedConvs.length ? [{ label: "pinned", items: pinnedConvs }] : []),
    ...groupByDate(unpinnedConvs),
  ];
  ...
  // "Sending" reflects only the chat currently being viewed — a stream running
  // in another chat shows its loader in the sidebar, not on this input.
  const isSending =
    sendMessage.isPending || (activeChatId !== null && streamingIds.has(activeChatId));

  function renderMessage(m: { id: string | number; role: string; content: string; sources?: string[] | null; created_at?: string | null; turn_id?: string | null; model?: string | null }, isPending?: boolean) {
    ...
    const timing = m.turn_id ? timings[m.turn_id] : undefined;
      ...
      <div key={m.id} className="group max-w-[720px] w-full mx-auto flex gap-3 animate-[msgIn_0.3s_ease]">
        ...
        <div className="flex-1">
          <div className="text-[0.78rem] font-semibold mb-1 flex items-center gap-2">
            <span className="font-mono">{m.model || "local-ai"}</span>
            <span className="font-mono text-[0.68rem] text-text-dim font-normal">{m.created_at ? formatTime(m.created_at) : "now"}</span>
            {timing && <ResponseTimer start={timing.start} end={timing.end} label={t("chat.responseTime")} />}
          </div>
          <div className={`inline-block text-left px-4 py-3 rounded-xl bg-bg-card border border-border rounded-tl-sm ${isThinking && !m.content ? "animate-pulse" : ""}`}>
            <div className="text-[0.92rem] leading-[1.7] text-text-muted font-light">
              {m.content
                ...
                : <span className="text-text-dim">{t("chat.thinking")}</span>}
            </div>
            {sourceList.length > 0 && (
  ...
  return (
    <div className="font-body bg-bg text-text h-screen overflow-hidden relative">
      ...
      <div className="flex h-screen">
        ...
        <aside className="w-[260px] bg-bg-elevated border-r border-border flex flex-col shrink-0">
                      ...
                      {menuOpenId === c.id && (
                        ...
                        >
                          ...
                          <button
                            type="button"
                            className="w-full flex items-center gap-2.5 px-3 py-2 text-[0.82rem] text-text-muted bg-transparent border-none cursor-pointer transition-colors hover:bg-bg-card hover:text-text text-left"
                            onClick={() => handleArchiveConversation(c.id)}
                          >
                            <Archive size={14} />
                            {t("chat.archive")}
                          </button>
          ...
          <div className="px-4 py-3 border-t border-border flex flex-col gap-2.5">
            <button
              type="button"
              className="text-text-dim bg-transparent border-none cursor-pointer text-[0.85rem] transition-colors flex items-center gap-2 hover:text-accent text-left p-0"
              onClick={() => setArchivedModalOpen(true)}
            >
              <Archive size={15} /> {t("chat.archivedChats")}
            </button>
            ...
            <div className="max-w-[720px] mx-auto relative">
                  ...
                  {mentionFiles.map((f) =>
                    f.id === ALL_MENTION_ID ? (
                      <button
                        key={f.id}
                        type="button"
                        className="w-full flex items-center gap-2.5 px-3 py-2 text-[0.82rem] text-accent bg-transparent border-none cursor-pointer transition-colors hover:bg-bg-card text-left"
                        onMouseDown={(e) => { e.preventDefault(); insertFileMention(f); }}
                      >
                        <span className="text-[0.85rem]">📚</span>
                        <span className="flex-1 truncate font-semibold">@all</span>
                        <span className="font-mono text-[0.65rem] text-text-dim">{t("chat.mentionAll")}</span>
                      </button>
                    ) : (
                      <button
                        key={f.id}
              ...
              <div className="flex flex-col w-full bg-bg-card border border-border rounded-xl min-h-[48px] max-h-[150px] overflow-y-auto pr-[4.5rem] pl-3 py-1 focus-within:border-border-focus transition-colors">
                ...
                <textarea
                  ...
                  onKeyDown={onKeyDown}
                  readOnly={isSending || isUploadingHere}
                />
              </div>
              <div className="absolute right-2.5 bottom-2.5 flex gap-1">
                ...
                {isSending ? (
                  ...
                  <button
                    ...
                    title={isUploadingHere ? t("chat.uploadingIndexing") : t("chat.send")}
                    onClick={() => handleSend()}
                    disabled={!input.trim() || isUploadingHere}
                  >
                    ↑
                  </button>
                )}
        ...
        <div className={`w-[300px] bg-bg-elevated border-l border-border flex-col shrink-0 ${filePanelOpen ? "flex" : "hidden"}`}>
          ...
          {embeddingModelsStatus && !embeddingModelsStatus.installed ? (
            ...
            <div
              className={`mx-3 mt-3 p-6 border border-dashed border-border rounded-[10px] text-center cursor-pointer transition-all hover:border-accent hover:bg-accent/15 ${isUploadingHere || embeddingBusy ? "pointer-events-none" : ""}`}
              ...
              onDrop={(e) => {
                e.preventDefault();
                e.stopPropagation();
                if (isUploadingHere || embeddingBusy) return;
                const fl = e.dataTransfer.files;
                if (!fl?.length) return;
                const list = Array.from(fl);
                // Create/resolve the chat ONCE, then index every dropped file
                // into that same conversation.
                void (async () => {
                  const cid = await ensureActiveChat();
                  if (cid == null) return;
                  list.forEach((f) => void processFileForUpload(f, cid));
                })();
              }}
              role="presentation"
            >
              {isUploadingHere && uploadProgress ? (
                <div className="bg-bg-card border border-border rounded-xl p-4 text-left">
                  <div className="flex justify-between font-mono text-xs text-text-muted mb-2 gap-2">
                    <span className="truncate">
                      {uploadProgress.phase === "indexing"
                        ? t("chat.fileUploadIndexing")
                        : t("chat.fileUploadUploading")}{" "}
                ...
                <>
                  ...
                  <div className="text-[0.82rem] font-light text-text-muted">{t("chat.dropFiles")}</div>
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  e.target.value = "";
                  // Create/resolve the chat ONCE, then index every selected
                  // file into that same conversation (not one chat per file).
                  void (async () => {
                    const cid = await ensureActiveChat();
                    if (cid == null) return;
                    list.forEach((file) => void processFileForUpload(file, cid));
                  })();
                }}
              />
            </div>
              ...
              <div>
                {ollamaModels.map((m) => (
                  <div
                    ...
                    onClick={() => selectChatModel(m)}
                    onKeyDown={(e) => {
                      if (e.key === "Enter") selectChatModel(m);
                    }}
                  >
      ...
      <CloudModelWarning
        modelName={cloudWarningName ?? ""}
        open={cloudWarningName !== null}
        onCancel={() => setCloudWarningName(null)}
        onConfirm={handleConfirmCloudPull}
      />
      ...
      {archivedModalOpen && (
        ...
        >
          <div
            className="bg-bg-elevated border border-border rounded-2xl w-full max-w-[520px] max-h-[80vh] overflow-y-auto p-7 shadow-[0_25px_80px_rgba(0,0,0,0.6)] relative animate-[cardIn_0.3s_ease]"
            onClick={(e) => e.stopPropagation()}
          >
            <button
              type="button"
            ...
            <h2 className="text-xl font-bold mb-1 flex items-center gap-2">
              <Archive size={18} /> {t("chat.archivedChats")}
            </h2>
            <p className="text-text-muted text-[0.88rem] font-light mb-5">{t("chat.archivedChatsDesc")}</p>

            {archivedConvs.length === 0 ? (
              <div className="py-10 text-center text-text-dim text-[0.88rem]">{t("chat.noArchived")}</div>
            ) : (
              <div className="flex flex-col gap-1.5">
                {archivedConvs.map((c) => (
                  <div
                    key={c.id}
                    className="flex items-center gap-2.5 px-3 py-2.5 rounded-lg border border-border hover:bg-bg-card transition-colors"
                  >
                    <span className="text-[0.8rem] opacity-50">💬</span>
                    <span className="flex-1 text-[0.88rem] whitespace-nowrap overflow-hidden text-ellipsis">{c.title}</span>
                    <button
                      type="button"
                      className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md border border-accent/40 text-accent bg-transparent text-[0.75rem] cursor-pointer transition-all hover:bg-accent/10"
                      onClick={() => handleUnarchiveConversation(c.id)}
                    >
                      <PinOff size={12} /> {t("chat.unarchive")}
                    </button>
                    <button
                      type="button"
                      className="inline-flex items-center gap-1 px-2 py-1 rounded-md border border-danger/30 text-danger bg-transparent text-[0.75rem] cursor-pointer transition-all hover:bg-danger/10"
                      onClick={() => handleDeleteConversation(c.id)}
                    >
                      <Trash2 size={12} />
                    </button>

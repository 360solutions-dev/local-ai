...
class DockerUsageView(APIView):
    ...
    def get(self, request):
        ...
        try:
            # v1.44 is the minimum API version newer Docker Engines accept.
            df = _docker_get("/v1.44/system/df")
        except Exception as e:
...
class ProviderTestView(APIView):
    ...
    # `host.docker.internal` and `gateway.docker.internal` are Docker Desktop's
    # built-in DNS names for reaching the host machine — required when users run
    # Ollama natively on macOS/Windows instead of the bundled container (faster,
    # uses host GPU). `ollama` is the in-network service for the bundled setup.
...
class ConnectivityView(APIView):
    """Check whether the Ollama model registry is reachable, so the UI can
    warn the user before a model pull that would otherwise fail on a machine
    with no internet. Pulls download from the public Ollama registry, so we
    probe it directly rather than relying on the browser's navigator.onLine
    (which is true even when connected to a network without internet)."""

    permission_classes = [IsAuthenticated]

    # Ollama's model registry endpoints. A HEAD/GET to either confirms the
    # machine can actually reach where models are downloaded from.
    _PROBE_URLS = ("https://registry.ollama.ai", "https://ollama.com")

    def get(self, request):
        for url in self._PROBE_URLS:
            try:
                resp = http_requests.head(url, timeout=4, allow_redirects=True)
...
class SwitchOllamaEngineView(_UpdaterBase):
    """Switch the Ollama engine between the host machine and the bundled Docker
    container. The updater (which has docker access) starts the container
    (engine=docker) or stops+removes it and its image (engine=machine); then we
    point the Ollama provider(s) at the matching endpoint so chat/RAG use it."""

    permission_classes = [IsAuthenticated]

    _MACHINE_URL = os.environ.get("OLLAMA_MACHINE_URL", "http://host.docker.internal:11434")
    _DOCKER_URL = os.environ.get("OLLAMA_DOCKER_URL", "http://ollama:11434")

    def post(self, request):
        engine = (request.data.get("engine") or "").strip()
        if engine not in ("machine", "docker"):
            return Response(
                {"error": {"code": "VALIDATION_ERROR", "message": "engine must be 'machine' or 'docker'."}},
                status=status.HTTP_400_BAD_REQUEST,
            )

        # Point the Ollama provider(s) at the new engine up-front; the updater
        # then starts/removes the container while we stream progress (SSE) so
        # the UI can show a progress bar like model pulls / uploads.
        new_endpoint = self._DOCKER_URL if engine == "docker" else self._MACHINE_URL
        Provider.objects.filter(type="ollama").update(endpoint=new_endpoint, is_connected=True)

        url = f"{self._UPDATER_URL.rstrip('/')}/engine-switch"
        ...
        def stream():
            try:
                ...
                payload = json.dumps({
                    "stage": "error",
                    "status": f"Engine service unavailable: {e}",
                    "percent": 0,
                })
                yield f"data: {payload}\n\n".encode()
...
class ProviderListCreateView(APIView):
    ...
    def get(self, request):
        ...
        # Auto-refresh Ollama connection status — but only downward.
        # If the user manually clicked "Disconnect", respect that and don't
        # silently reconnect on the next page poll. We only update
        # is_connected: True → False (legitimate auto-detect of an outage).
        # Going False → True requires the user to explicitly click Connect.
        for provider in Provider.objects.filter(type="ollama"):
            if not provider.is_connected:
                continue  # respect manual disconnect
